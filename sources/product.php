...
if ($id != '') {
    ...
    if($rowDetail and $rowDetail['type'] == 'san-pham') {
        ...
        if(isset($_COOKIE['recently_viewed'])) {
            $sql = "select photo, name$lang, slugvi, slugen, sale_price, regular_price, discount, id from #_product where type = 'san-pham' and CONCAT(',',id, ',') REGEXP ',(".str_replace(',', '|', implode(',', json_decode($_COOKIE['recently_viewed'], true)) )."),' and find_in_set('hienthi',status) order by numb, id desc";
