...
$products = $cache->get("select name$lang, slug$lang, id, photo from #_product where type = ? and status = ? ORDER BY numb, id desc", array('san-pham', 'hienthi'), 'result', 7200);

$introduce = $cache->get("select name$lang, content$lang from #_static where type = ? and status = ?", array('gioi-thieu', 'hienthi'), 'fetch', 7200);

$ability = $cache->get("select name$lang, content$lang from #_static where type = ? limit 0,1", array('ability'), 'fetch', 7200);

$tech = $cache->get("select name$lang, content$lang from #_static where type = ? limit 0,1", array('tech'), 'fetch', 7200);
