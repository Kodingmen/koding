...
/**
 * Get all course level 1 (List)
 */
$courseList = $d->rawQuery("select id, name$lang, slug$lang from #_news_list where type = ? and find_in_set('hienthi',status)", array('khoa-hoc'));
/**
 * Get highlight courses
 */
$highlightCourses = $d->rawQuery("select id, name$lang, slug$lang, photo, link_video from #_news where type = ? and find_in_set('hienthi',status) and find_in_set('noibat',status)", array('khoa-hoc'));
