...
<?php if($type == 'video-thumbnail-slick') {
...
if ($type == 'video-fotorama') {
    ...
    if (count($video_home)) { ?>
        <div id="fotorama-videos" data-width="100%" data-thumbmargin="10" data-height="330" data-fit="cover" data-thumbwidth="140" data-thumbheight="80" data-allowfullscreen="true" data-nav="thumbs">
            <a href="<?=THUMBS?>/500x500x2/<?= UPLOAD_PRODUCT_L ?>tin-tuc-1-3013.png">
                <img src="<?=THUMBS?>/500x500x2/<?= UPLOAD_PRODUCT_L ?>tin-tuc-1-3013.png" width="144" height="96">
            </a>
...
if ($type == 'photo-video-fotorama') {
    $video_home = $d->rawQuery("select link_video, id, name$lang from #_photo where type = ? and act <> ? and find_in_set('noibat',status) and find_in_set('hienthi',status) order by numb, id desc", array('video', 'photo_static'));
    $photo_home = $d->rawQuery("select photo from #_gallery where find_in_set('hienthi',status) order by numb,id desc");
    if (count($video_home) || count($photo_home)) { ?>
        <div id="fotorama-photos-videos" data-width="100%" data-shuffle="true" data-thumbmargin="10" data-height="330" data-fit="cover" data-thumbwidth="140" data-thumbheight="80" data-allowfullscreen="true" data-nav="thumbs">
            <?php if (count($video_home)) { foreach ($video_home as $k => $v) { ?>
                <a href="https://youtube.com/watch?v=<?= $func->getYoutube($v['link_video']) ?>" title="<?= $v['name' . $lang] ?>"></a>
            <?php } } ?>
            <?php if (count($photo_home)) { foreach ($photo_home as $k => $v) { ?>
                <a href="<?=THUMBS?>/500x500x2/<?= UPLOAD_PRODUCT_L . $v['photo'] ?>">
                    <img src="<?=THUMBS?>/500x500x2/<?= UPLOAD_PRODUCT_L . $v['photo']?>">
                </a>
            <?php } } ?>
        </div>
    <?php }
...
if ($type == 'fanpage-facebook') { ?>
    <div class="fb-page" data-href="<?= $optsetting['fanpage'] ?>" data-tabs="timeline" data-width="400" data-height="210" data-small-header="true" data-adapt-container-width="true" data-hide-cover="false" data-show-facepile="true">
