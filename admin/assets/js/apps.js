...
function deleteImageStatic(url) {
	document.location = url;
}
/* HoldOn */
function holdonOpen(
		...
		buttons: {
			success: {
				...
				action: function () {
					...
					if (action == 'remove-static-image') deleteImageStatic(value);
				}
			},
			cancel: {
...
$(document).ready(function () {
	...
	$(document).on('click', '.delete_1hinh', function(event) {
		event.preventDefault();
		var ele = $(this);
		$.confirm({
			title: 'Thông báo',
