...
function clearCache($pathCache = ROOT . "caches/data"){
    if (!is_dir($pathCache)) return false;
        if ($handle = opendir($pathCache)) {
            while (false !== ($file = readdir($handle))) {
                if ($file != "." && $file != ".." && $file != ".htaccess" && $file != "thumb.db" && $file != "index.html") {
                    if (!file_exists($pathCache . "/" . $file) || !is_readable($pathCache . "/" . $file)) return false;
                    if(is_dir($pathCache . "/" . $file)){
                        clearCache($pathCache . "/" . $file);
                    }
                    unlink($pathCache . "/" . $file);
                }
            }
            return true;
        } else return false;
...
function saveMan()
{
    ...
    $option2 = json_decode($row_opt['options'],true);
    ...
    if ($data) {
        ...
        if (!empty($config['news'][$type]['video']) && $config['news'][$type]['video']) {
            $data['link_video'] = $_POST['data']['link_video'] ?? '';
        }
