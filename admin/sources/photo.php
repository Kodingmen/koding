...
if (isset($config['photo'])) {
    $arrCheck = array();
    $actCheck = '';
    if ($act == 'photo_static_remove' || $act == 'photo_static' || $act == 'save_static' || $act == 'save-watermark' || $act == 'preview-watermark') $actCheck = 'photo_static';
...
/* Remove photo */
function removePhotoStatic()
{
    global $d, $func, $curPage, $items, $paging, $type;
    $row = $d->rawQueryOne("select id, photo from #_photo where act = ? and type = ? limit 0,1", array('photo_static', $type));
    if($row) {
        $func->deleteFile(UPLOAD_PHOTO . $row['photo']);
        $data['photo'] = '';   
        $d->where('id', $row['id']);
...
function savePhoto()
{
    global $d, $func, $flash, $curPage, $config, $type, $cache;
    /* Check post */
    if (empty($_POST)) {
        $func->transfer("Không nhận được dữ liệu", "index.php?com=photo&act=man_photo&type=" . $type . "&p=" . $curPage, false);
    }
